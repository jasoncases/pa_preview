 ...
 * customMobile
 *   - default: false
 *   - if true, when window.innerWidth is < 900, repositions the
 *     daterangepicker parent element to a more central modal location
 *     and creates a shadowed background to allow the calendar modal
 *     to pop to the foreground. `modal-blocker` is the class/data-id
 *     of the shadowed background, it is located at
 *     View/template/footer.html
 *   - changes made: had to add if blocks to show(), hide(), and move()
 *     anchors are #customMobile-1, #customMobile-2, & #customMobile-3,
 *     respectively
 */
; ( function ( root, factory ) {
  ...
  var DateRangePicker = function ( element, options, cb ) {
    ...
    this.removeButtonClasses = 'btn-default'
    ...
    if ( typeof options.locale === 'object' ) {
      ...
      //* Added Remove
      if ( typeof options.locale.removeLabel === 'string' )
        this.locale.removeLabel = options.locale.removeLabel
    ...
    /**
     * customMobile is custom for Proaction. It changes the top/left
     * values of the mobile daterangepicker parent element and creates
     * a black background to allow the modal to stand out from the
     * canvas
     */
    if ( typeof options.customMobile === 'boolean' )
      this.customMobile = options.customMobile
    ...
    //* Added remove
    if ( typeof options.removeButtonClasses === 'string' )
      this.removeButtonClasses = options.removeButtonClasses
    //* Added remove
    if ( typeof options.removeClass === 'string' )
      this.removeButtonClasses = options.removeClass
    ...
    this.container
      .find( '.applyBtn, .cancelBtn, removeBtn' )
      .addClass( this.buttonClasses )
    if ( this.applyButtonClasses.length )
      this.container.find( '.applyBtn' ).addClass( this.applyButtonClasses )
    if ( this.cancelButtonClasses.length )
      this.container.find( '.cancelBtn' ).addClass( this.cancelButtonClasses )
    if ( this.removeButtonClasses.length )
      this.container.find( '.removeBtn' ).addClass( this.removeButtonClasses )
    ...
    if ( !this.removeBtn ) {
      this.container.find( '.removeBtn' ).hide()
    }
  ...
  DateRangePicker.prototype = {
    ...
    move: function () {
      ...
      if ( this.opens == 'left' ) {
        ...
        if ( containerLeft + containerWidth > $( window ).width() ) {
          ...
          // #customMobile-3
          if ( this.customMobile && $( window ).width() < 800 ) {
            this.container.css( {
              top: 'calc(50% - 269px)',
              left: 'calc(50% - 139px)',
            } )
    ...
    show: function ( e ) {
      ...
      if ( this.customMobile ) {
        if ( $( window ).width() < 800 ) {
          const modalBlocker = document.querySelector(
            '[data-id="modal-blocker"]'
          )
          if ( modalBlocker ) modalBlocker.style.display = 'block'
        }
      }
      /**
       * * PONTENTIAL POINT OF INTEREST
       */
    ...
    clickRemove: function ( e ) {
      this.hide()
      this.element.trigger( 'removeBtn.daterangepicker', this )
    },
    ...
    updateElement: function () {
      if ( this.element.is( 'input' ) && this.autoUpdateInput !== false ) {
